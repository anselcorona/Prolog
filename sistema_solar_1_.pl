...
%Dia mas corto que tierra

dia_mas_corto_no_gaseoso(Planeta):-
	rotacion(tierra,Tiempo_tierra),
	rotacion(X,Y),
	Y<Tiempo_tierra,
	planeta(X,L,_,_),
	not(miembro(gaseoso,L)),
	Planeta = X.

%Tiempo Traslacion Mas de un satelite

tiempo_traslacion_mas_de_un_satelite(Tiempo_Traslad):-
	satelites(X,L),
	longitud(L,N),
	N>1,
	traslacion(X,Y),
...
%Cantidad de planetas a cruzar

cuantos_planetas_debo_cruzar(Planeta1, Planeta2, Resultado):-
	planeta(Planeta1, _, _, X),
	planeta(Planeta2, _, _, Y),
	Z is Y-X,
	Resultado is abs(Z).

%Planetas Teluricos sin satelites.

teluricos_sin_satelites:-
	planeta(X,L,_,_),
	miembro(telurico,L),
	satelites(X,L2),
	longitud(L2,0).
